...
export class AppComponent {
  ...
  constructor(private tempoService: TempoService) {
    ...
    this.getCurrentLocation().pipe().subscribe(data => {
      coordenadas = data
      this.tempoService.getCurrentWeatherByCoords({ latitude: coordenadas.latitude, longitude: coordenadas.longitude }
        ).subscribe(novoTempo => this.tempoAtual = novoTempo)
    })
  }
  ...
  getCurrentLocation(): Observable<GeolocationCoordinates> {
    return new Observable<GeolocationCoordinates>((observer) => {
      window.navigator.geolocation.getCurrentPosition(
        ...
        (error) =>  alert(`ERROR(${error.code}): ${'errouu'}`)
      );
    }).pipe(
      retry(1),
